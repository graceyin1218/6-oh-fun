...
#hartz, fall 2012
def solve_equations(eqnlist, verbose = False):
    """
    ...
    x = gauss_solve(A,b, verbose=verbose)

    #create and return mapping from variable names to solutions
    index_to_name = {ix:n for (n,ix) in name_to_index.items()}
    if verbose:
        print()
        print('Solution:')
        for ix in range(len(x)):
            print('%s = %f' % (index_to_name[ix], x[ix]))
    return {index_to_name[ix]:x[ix] for ix in range(len(x))}
